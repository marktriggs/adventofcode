...
// Note: Setting f to 0 ended up being important
...
fn day23_pt1() {
    let instructions: Vec<&str> = DAY23_INPUT.trim().split("\n").map(|s| {
        if &s[0..1] == "#" {
            "nop"
        } else {
            s
    ...
    let mut registers = "abcdefgh".chars().fold(HashMap::new(), |mut acc, register| {
        if register == 'a' {
...
#[derive(Debug)]
struct State {
    last_connector: usize,
    used_components: usize,
}

impl State {
    fn next(&self, component_used_idx: usize) -> State {
        let used_component = COMPONENTS[component_used_idx].clone();

        let last_connector = if used_component.0 == self.last_connector {
            used_component.1
        } else {
            used_component.0
        };
        ...
        State {
            ...
            used_components: (self.used_components | (1 << component_used_idx))
        }
    }

    fn used_component(&self, component_id: usize) -> bool {
        (self.used_components & (1 << component_id)) != 0
    }

    fn component_fits(&self, component_id: usize) -> bool {
        let component = COMPONENTS[component_id];

        self.last_connector == component.0 || self.last_connector == component.1
    }
    ...
    fn possible_next_states(&self) -> Vec<State> {
        ...
        for component_id in 0..COMPONENTS.len() {
            if !self.used_component(component_id) && self.component_fits(component_id) {
                result.push(self.next(component_id));
            }
        }
    ...
    fn strength(&self) -> usize {
        ...
        for component_id in 0..COMPONENTS.len() {
            if self.used_component(component_id) {
                result += COMPONENTS[component_id].0;
                result += COMPONENTS[component_id].1;
            }
        }
...
fn day24_pt1() {
    ...
    let mut possible_states = vec!(State {last_connector: 0, used_components: 0 });

    while !possible_states.is_empty() {
        let mut next_possible_states = Vec::new();

        for p in possible_states {
            let next = p.possible_next_states();

            if next.is_empty() {
                final_states.push(p);
            } else {
                next_possible_states.extend(next);
            }
        }
    ...
    println!("Strongest bridge {}", final_states.iter().map(|state| state.strength()).max().unwrap());
}
...
fn day24_pt2() {
    ...
    println!("Longest bridge {}", final_states.iter().map(|state| state.strength()).max().unwrap());
}
...
fn day25_input() -> HashMap<String, Vec<Step>> {
    let mut states = HashMap::new();

    states.insert("a".to_owned(), vec!(Step {write: 1, move_to: 1, next_state: "b".to_owned() },
                                       Step {write: 0, move_to: -1, next_state: "d".to_owned() }));

    states.insert("b".to_owned(), vec!(Step {write: 1, move_to: 1, next_state: "c".to_owned() },
                                       Step {write: 0, move_to: 1, next_state: "f".to_owned() }));

    states.insert("c".to_owned(), vec!(Step {write: 1, move_to: -1, next_state: "c".to_owned() },
                                       Step {write: 1, move_to: -1, next_state: "a".to_owned() }));

    states.insert("d".to_owned(), vec!(Step {write: 0, move_to: -1, next_state: "e".to_owned() },
                                       Step {write: 1, move_to: 1, next_state: "a".to_owned() }));

    states.insert("e".to_owned(), vec!(Step {write: 1, move_to: -1, next_state: "a".to_owned() },
                                       Step {write: 0, move_to: 1, next_state: "b".to_owned() }));

    states.insert("f".to_owned(), vec!(Step {write: 0, move_to: 1, next_state: "c".to_owned() },
                                       Step {write: 0, move_to: 1, next_state: "e".to_owned() }));

    states
}

fn day25() {
    let input = day25_input();
    let mut current_state = "a";
    let mut tape: [usize; 1000000] = [0; 1000000];
    let mut i = 500000;

    for _ in 0..12302209 {
        let step = &input[&(current_state.to_owned())][tape[i]];

        tape[i] = step.write;
        i = (i as i64 + step.move_to) as usize;
        current_state = &step.next_state;
    }

    println!("Bits set: {}", tape.iter().filter(|&&i| i == 1).count());
}
